...
export const signin = (req, res) => {
    ...
    hash(password, null).then(password => {
        User.findOne({ emailId, password }).then(async user => {
            console.log(`333333`, user.token)
            hash(user.token, 1800).then(authToken => res.send(authToken)).catch(e => {console.log(e);res.status(500).send(e)})
